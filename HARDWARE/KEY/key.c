 ...
 * @LastEditTime: 2020-12-21 20:22:20
 */
#include "key.h"
#include "stm32f10x.h"
#include "sys.h"
...
u8 KEY_Scan(u8 mode)
{
    ...
    if (key_up && (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_5) == 0 || GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_8) == 0 || GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_9) == 0 || GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0) == 1)) {
        ...
        if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_5) == 0)
            ...
            return WKUP_PRES;
    } else if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_5) == 1 && GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_8) == 1 && GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_9) == 1 && GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0) == 0)
        key_up = 1;
    return 0; // 无按键按下
}
